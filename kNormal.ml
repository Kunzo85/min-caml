...
let rec g env e = (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  (* let t' = *)
    ...
    | Syntax.Not(e') -> 
        g env (make_wloc 
                (Syntax.If(e', make_wloc (Syntax.Bool(false)) e'.loc, make_wloc (Syntax.Bool(true)) e'.loc)) 
                e'.loc)
    | Syntax.Neg(e') ->
        insert_let (g env e')
    ...
    | Syntax.Eq _ | Syntax.LE _ as cmp ->
        g env (make_wloc
                (Syntax.If(make_wloc cmp e.loc, make_wloc (Syntax.Bool(true)) e.loc, make_wloc (Syntax.Bool(false)) e.loc))
                e.loc)
    | Syntax.If({ node = Syntax.Not _; _} as e1 , e2, e3) -> 
        g env (make_wloc (Syntax.If(e1, e3, e2)) e.loc) (* notによる分岐を変換 (caml2html: knormal_not) *)
    ...
    | Syntax.If(e1, e2, e3) -> 
        g env (make_wloc 
                (Syntax.If(make_wloc
                            (Syntax.Eq(e1, make_wloc (Syntax.Bool(false)) e.loc))
                            e.loc,
                          e3, e2))
                e.loc) (* 比較のない分岐を変換 (caml2html: knormal_if) *)
